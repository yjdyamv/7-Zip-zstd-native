...
The **musl-static** variants are fully static (built natively in an Alpine
container, `LDFLAGS_STATIC_2=-static`), so a single binary runs on **both**
glibc and musl (Alpine) hosts — glibc-only deployments may use either; pick
the musl-static archive to cover Alpine too.
